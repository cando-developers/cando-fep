...
(defun parse-residue-groups (mol-string)
  (multiple-value-bind (groups map-names-numbers)
      (jsme:parse-groups mol-string)
    (let (core
...
(defun build-topologys (mol-string)
  (let ((side-chains (make-hash-table))
    ...
    (multiple-value-bind (core-group side-chain-groups map-names-numbers)
        (parse-residue-groups mol-string)
      (multiple-value-bind (core-residue core-vectors)
          (build-core-residue core-group)
        (let ((core-topology (cando::make-topology-from-residue core-residue)))
          ...
          (loop for side-chain-name being the hash-keys in side-chain-groups using (hash-value side-chain-group)
                ...
                do (let* ((side-chain-topology (cando::make-topology-from-residue side-chain-residue))
  ...
  (let ((top-groups nil))
    ...
    (let ((cartesian (cartesian top-groups)))
      (loop for cart in cartesian
            ...
            for core-residue = (let ((res (chem:build-residue core-topology)))
            ...
            collect (progn
                      ...
                      (loop for (name . top) in cart
                            for side-residue = (let ((res (chem:build-residue top)))
